...
//Setting values for scroll lock indicator
uint8_t ledStatus;
uint8_t ledStatusO = 8;
...
void setup() {
  ...
  System.begin();
  //Srart keyboard to get caps num and scroll lock
  Keyboard.begin();
...
void uLock(){
    ...
    if(ledStatus != ledStatusO){
      ...
      display.setTextSize(1);
      display.getTextBounds(wrd, 0, 0, &x1, &y1, &w1, &h1);
      display.setCursor(2,SHE-h1-2);
      display.fillRect(2,SHE-h1-2,18,h1, BLACK); 
      display.print(wrd);
      ledStatusO = ledStatus;
      display.display();
  }
}
